...
bool sortSpecificsEntityUser(KAEntity* ent1,KAEntity* ent2){
	Specific *sp1=dynamic_cast<Specific*>(ent1);
	Specific *sp2=dynamic_cast<Specific*>(ent2);
	return sp1->name<sp2->name;
}
Specifics::Specifics(SDODBImage *parent){
	this->parent=parent;
	sortFuncU=sortSpecificsEntityUser;
}
...
bool sortRoomsEntityUser(KAEntity *a,KAEntity *b){
...
bool sortGroupsEntityUser(KAEntity *a,KAEntity *b){
...
Groups::Groups(SDODBImage *parent){
	this->parent=parent;
	sortFuncU=sortGroupsEntityUser;
}
	...
	if(diff&4){
		...
		if(remove.size()>0){
			...
			for(int i=0;i<remove.size()-1;++i){
				spec=dynamic_cast<Specific*>(remove[i]);
				query+=String().sprintf(L"%d,",spec->getID());
				addString+=String().sprintf(L"('%s',%d,Now(),%d,'DELETE'),",L"specprogram",parent->parent->getUID(),this->id);
				it=std::find(specifics.begin(),specifics.end(),remove[i]);
				specifics.erase(it);
			}
			spec=dynamic_cast<Specific*>(remove[remove.size()-1]);
...
///CHECK ME ON THE PRACTICE
		...
		for(int i=0;i<this->dates.size() && temp;++i){
			...
			for(int j=0;j<ent->dates.size();++j)
				if(this->dates[i]==ent->dates[j]){
					temp=true;
					break;
				}
		}
		if(!temp)result=result|2;
